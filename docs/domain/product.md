...
## 할인 정책 (프론트 하드코딩 — `src/shared/utils/constants.js`)

### 일반 상품 수량 할인 (`QUANTITY_DISCOUNTS`)

수량 선택 박스 UI로 표시. 옵션 선택과 함께 노출.
...
### 정기배송 주기별 할인 (`CYCLE_DISCOUNTS`)

배송주기 선택 박스 UI로 표시. 배송주기 드롭다운과 함께 노출.
...
> 주기별 할인 금액은 결제 단계에서 적용 예정. 상세 페이지에서는 UI 표시 용도.

---

## 상품 유형별 상세 페이지 UI 분기

`product.isSubscribable` 값에 따라 UI 전체가 분기된다.

### isSubscribable = false (일반 상품)

```
맛 옵션 드롭다운 (product.options)
수량 할인 티어 박스 (QUANTITY_DISCOUNTS: 단품 / 5개이상 5% / 10개이상 10%)
──────────────────────────────────
총 금액 | 장바구니  구매하기 → /checkout
```
...
맛 옵션 드롭다운 (product.options, 있을 때만)
...
- 장바구니·구매하기 버튼 모두 동일하게 `/checkout` (일반결제 페이지)로 이동
- 배송주기 미선택 시 `cycleError` 표시, 옵션 미선택 시 `optionError` 표시
...
| `isSubscribable` | `isSubscribable` | 정기배송 가능 여부 |
| `deliveryCycles[].value` | `deliveryCycles[].value` | 배송주기 코드 (`1w`/`1m`/`2m`/`3m`) |
| `deliveryCycles[].label` | `deliveryCycles[].label` | 배송주기 표시명 |
...
```js
{
  ...
  deliveryCycles: [{ value, label }],  // isSubscribable=true일 때만 데이터 존재
...
### isSubscribable = false (일반 상품)
- 맛 옵션 드롭다운 → 수량 스텝퍼(+/-) → 총 금액 = `(price + optionExtra) × qty`
- "함께 구매하면 좋은 제품" 섹션 표시
- CTA: 장바구니 / 구매하기

### isSubscribable = true (정기배송 상품)
- 배송주기 드롭다운 (`product.deliveryCycles` 데이터 기반, 백엔드 동적 제공)
- 맛 옵션 드롭다운 (options 있을 때)
- 수량 선택 그리드 (`product.bundleOptions`)
- 총 금액 = `bundleOptions[selectedQty].price + optionExtra`
- CTA: 장바구니 / 구매하기
